...
//    heap[(2 * i) + 2]: Returns the right child node
    ...
    public MaxHeap(){
        ...
        this.heap = (T[]) Array.newInstance(Comparable[].class.getComponentType(),
                 DEFAULT_CAPACITY);
    }
    ...
    private void heapifyUp() {
        //step 1: start from end of the heap as a current node
        int index = size -1;

        // step 2: while the current node has a parent
        // and parent is less then the current node
        // then swap these nodes
    ...
    private void heapifyDown() {
        // Step 1: start from root of heap as the current node
        int index = 0;
    ...
    public void printHeap() {
        System.out.println("Root Node " + heap[0] + "\n");
        for(int i=0; i<size; i++) {
            System.out.println("Node: " + heap[i]);
            if(hasLeftChild(i)) {
                System.out.println(" Left node: " + heap[getLeftChildIndex(i)]);
            }
            if(hasRightChild(i)) {
                System.out.println(" Right node: " + heap[getRightChildIndex(i)]);
            }
            System.out.println();
        }
        System.out.println();
